...
function createTrainingData(context) {
    ...
    const labels = []
    context.users
        .filter(user => user.purchases.length) // só usuários com compras (para ter vetor de usuário)
        .forEach(user => {
...
async function configureNeuralNetAndTrain(trainData) {
    ...
    // Camada de entrada
    // - inputShape: Número de features por exemplo de treino (trainData.inputDimension)
    ...
    model.add(
        tf.layers.dense({
            units: 1,
            activation: 'sigmoid' // saída entre 0 e 1 (probabilidade de compra)
        })
    )

    model.compile({
        optimizer: tf.train.adam(0.01), // algoritmo de otimização (ajusta os pesos) 
        loss: 'binaryCrossentropy', // função de perda para classificação binária
        metrics: ['accuracy'] // métrica para avaliar desempenho
    })    
...
function recommend({ user }) {
    ...
    // Consulta: Encontre os s00 produtos mais próximos do vetor
